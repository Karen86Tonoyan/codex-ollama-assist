...
const MAX_INLINE_DATA_URL = 64 * 1024

export function sanitizeStorageForExport(
  ...
  return Object.fromEntries(
    Object.entries(storage).filter(([key, value]) => {
      ...
      if (
        ...
        value.length > MAX_INLINE_DATA_URL
      ) {
        return false
      }
      return true
...
export const importPageAssistDataFromObject = async (
  ...
  if (
    ...
    await browser.storage.local.set(
      sanitizeStorageForExport(data.storageLocal)
    )
  }
  ...
  if (wants("storageSync")) {
    ...
    if (
      ...
      await browser.storage.sync.set(
        sanitizeStorageForExport(data.storageSync)
      )
    }
  }
}
